...
string ValType_name[10] = {"bool", "int", "char", "string","void","const int","const char","8"};//对应enum ValueType
//-------------
...
{
    if(cur_tb->tmap.count(t->var_name)==0)
    {
        cur_tb->tmap[t->var_name]=make_pair(ValType_name[t->father->type->type],n_top);
        cur_tb->nodemap[t->var_name]=t;
        n_top++;
    }
    else
    {
