...
void TreeNode::stmt_get_label(TreeNode *t)//set_data_seg_AND_get_label
{
	...
	{									//     p          q
		...
		if(t->child->type->type==VALUE_INT||t->child->type->type==VALUE_CHAR||t->child->type->type==VALUE_CONST_INT||t->child->type->type==VALUE_CONST_CHAR)
		{
			TreeNode* p=t->child->child,*q=nullptr;
			if(p->child->sibling!=nullptr)
				q=p->child->sibling;//int a[5];  int a[5]={...};

			int NUM=p->arr_len,initNUM=p->init_len;
			
			// while (p!=nullptr)//int a[3].b[4]...
			// {
				// if(p->child!=nullptr)//int a=1;
				// {
			vardata[var_i]="\t.align 4\n";
...
void TreeNode::stmt_gen_code(TreeNode *t)
{
	...
	if(t_type==STMT_PRINTF)
	{
		...
		else
		{
			...
			else if(p->sibling->child->sibling==nullptr)//printf("%d",a)//注意第二个孩子是block
			{
				...
				{
					if(p->sibling->child->index_flag=='i')
					cout<<"\tmovl\t"<<p->sibling->child->var_name<<"+"<<p->sibling->child->index*4<<",%eax\n";//p->sibling->childs is arg 2,3,4...
					else
					{
						cout<<"\tmovl\t"<<p->sibling->child->var_index->var_name<<", %edi\n";
						cout<<"\tmovl\t"<<p->sibling->child->var_name<<"(,%edi,4),%eax\n";
					}
			...
			else//注意参数压栈顺序  //printf("%d%d%d%d%d",a,b,c,d,e)
			{
				TreeNode* tmp;
				TreeNode* a[20];
				tmp=p->sibling->child;
	...
	{
		string src,dst=p->var_name;
		if(p->nodeType==NODE_ARR)
		{
			if(p->index_flag=='i')
			{
				cout<<"\tmovl\t$"<<p->index<<", %edi\n";
				dst=p->var_name+"(,%edi,4)";
			}
			else
			{
				cout<<"\tmovl\t"<<p->var_index->var_name<<", %edi\n";
				dst=p->var_name+"(,%edi,4)";
			}
		...
		if(p->sibling->nodeType==NODE_CONST)//a=1
		{
			if(p->sibling->val_type_flag=='i')
				cout<<"\tmovl\t$"<<p->sibling->int_val<<", "<<dst<<endl;
			else if(p->sibling->val_type_flag=='c')//---------------movb!!!
				cout<<"\tmovb\t$"<<int(p->sibling->ch_val)<<", "<<dst<<endl;
			else if(p->sibling->val_type_flag=='b')
				cout<<"\tmovl\t$"<<bool(p->sibling->int_val)<<", "<<dst<<endl;
			else ;
			
		}
		else if(p->sibling->nodeType==NODE_VAR)//a=b
		{
			cout<<"\tmovl\t"<<p->sibling->var_name<<", "<<dst<<endl;
		}
		else if(p->sibling->nodeType==NODE_EXPR)//a=2+1
		{
			//let expr'res be %eax
			expr_gen_code(p->sibling);
			cout<<"\tmovl\t%eax, "<<dst<<endl;
		}
